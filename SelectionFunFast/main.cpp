...
string firstOf3(string const &a, string const &b, string const &c) {
    ...
    if (length_b < min_length) {min_length = length_b; return_string = b;}
    if (length_c < min_length) {min_length = length_c; return_string = c;}
    for (int i = 0; i < min_length; i++) {
        char a_char = a.at(i);
        char b_char = b.at(i);
        char c_char = c.at(i);
        char min_char = a_char;
        if (b_char < min_char) {min_char = b_char; return b;}
        if (c_char < min_char) {min_char = c_char; return c;}
...
int main() {
// Use the ascii character set where  "A" < "a" and "1" < "A"
    assert(firstOf3("123", "123 0", "1232") == "123");
    assert(firstOf3("AAA", "aaa", "bbb") == "AAA");
    assert(firstOf3("1234", "AAA", "aaa") == "1234");
    assert(firstOf3("zxc", "zxcb", "zxcv") == "zxc");
    assert(firstOf3("", "", "") == "");
    assert(firstOf3("", "", " 1") == "");
    assert(firstOf3("121", "123", " 122") == " 122");
    return 0;
}
